...
class ChatStore {
	...
	/** Thinking mode toggle - controls whether reasoning/thinking is enabled for chat completions */
	enableThinking = $state(false);
	...
	private async streamChatCompletion(
		...
		// Track last client-side timings for saving when flow completes (vLLM doesn't return server timings)
		let lastClientTimings: ChatMessageTimings | undefined;
		...
		const streamCallbacks: ChatStreamCallbacks = {
			...
			onTimings: (timings?: ChatMessageTimings, promptProgress?: ChatMessagePromptProgress) => {
				...
				// Track client-side timings for saving on flow complete (vLLM doesn't return server timings)
				if (timings?.predicted_n && timings?.predicted_ms) {
					lastClientTimings = timings;
				}
			},
			...
			onFlowComplete: (finalTimings?: ChatMessageTimings) => {
				// Use server timings if available, otherwise fall back to client-side tracked timings (vLLM)
				const timingsToSave = finalTimings || lastClientTimings;
				if (timingsToSave) {
					const idx = conversationsStore.findMessageIndex(assistantMessage.id);

					conversationsStore.updateMessageAtIndex(idx, { timings: timingsToSave });
					DatabaseService.updateMessage(assistantMessage.id, { timings: timingsToSave }).catch(
						console.error
					);
				}
		...
		await ChatService.sendMessage(
			allMessages,
			{
				...
				onComplete: async (
					...
					// Use server timings if available, otherwise fall back to client-side tracked timings (vLLM)
					const timingsToSave = timings || lastClientTimings;
					const updateData: Record<string, unknown> = {
						content,
					...
					if (timingsToSave) uiUpdate.timings = timingsToSave;
					if (resolvedModel) uiUpdate.model = resolvedModel;
					conversationsStore.updateMessageAtIndex(idx, uiUpdate);
					await conversationsStore.updateCurrentNode(currentMessageId);
					cleanupStreamingState();
	...
	private getApiOptions(): Record<string, unknown> {
		...
		apiOptions.enableThinking = this.enableThinking;
