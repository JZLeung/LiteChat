...
class ModelsStore {
	 ...
	 * Checks in order: model.modalities, modelPropsCache, then derives from capabilities array.
	 * The capabilities fallback is important for non-llama.cpp backends (e.g., vLLM) where
	 * /props returns 404 and fallback props have vision=false.
	 */
	getModelModalities(modelId: string): ModelModalities | null {
		...
		// Fallback: derive modalities from capabilities array (e.g., vLLM / OpenAI-compatible backends)
		...
		// Final fallback: default to true for both modalities.
		// Many backends (e.g., vLLM) don't report capabilities/modalities in /v1/models,
		// but the model may still support vision/audio. Defaulting to true allows the UI
		// to show the options; the server will reject unsupported attachments if needed.
