...
import { APP_NAME } from '$lib/constants';
	...
	let hasAudioModality = $derived.by(() => {
		// Default to true when no model is selected yet (e.g., ROUTER mode before selection).
		// Many backends (e.g., vLLM) don't report modalities, so defaulting to true is safer.
		if (activeModelId) {
